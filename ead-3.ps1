        ...
        switch ($emulatorName) {
			...
			"Project64" {
				  ...
				  "Dolphin" {
					...
					# Append 'Dolphin' to the download path
					...
					# URL for the Dolphin Development versions download page
					...
					try {
						$webContent = Invoke-WebRequest -Uri $dolphinUrl -Headers $headers
						#Write-Host "Successfully fetched Dolphin download page."
					} catch {
						Write-Error "Failed to fetch the Dolphin Emulator download page. $_"
						exit 1
					}

					# Locate the <div id="download-dev"> section where Development versions are listed
					$downloadDevSection = $webContent.ParsedHtml.getElementById("download-dev")

					if (-not $downloadDevSection) {
						Write-Error "Could not find the Development versions section on the page."
						exit 1
					}

					# Find all the download links in the 'download-dev' section, and grab the FIRST 'Windows x64' link
					$windowsDownloadLink = $null
					$foundLinks = $downloadDevSection.getElementsByTagName('a') | Where-Object { $_.innerText -match "Windows x64" }

					if ($foundLinks.Count -gt 0) {
						# Take the first 'Windows x64' link (most recent)
						$windowsDownloadLink = $foundLinks[0].href
					}

					if (-not $windowsDownloadLink) {
						Write-Error "No download link found for the Windows x64 version in Development versions."
						exit 1
					}

					# Construct the full download URL
					$downloadUrl = $windowsDownloadLink
					#Write-Host "Download URL for Windows x64: $downloadUrl"

					# Define the destination file path for the download
					$fileName = [System.IO.Path]::GetFileName($downloadUrl)
					$destinationFilePath = Join-Path -Path $downloadPath -ChildPath $fileName

					#Write-Host "Downloading to: $destinationFilePath"

					# Start the BITS transfer to download the file
					try {
						Start-BitsTransfer -Source $downloadUrl -Destination $destinationFilePath -Priority Foreground
						Write-Host "Dolphin download for Windows x64 completed successfully." -ForegroundColor "Green" -BackgroundColor "Black"
					} catch {
						Write-Error "Failed to download the file using BITS: $_"
						exit 1
					}
				...
				"a7800" {
						# a7800 Download Logic for Mac
							...
							"Dolphin" {
								...
								# Find all the download links in the 'download-dev' section, and grab the FIRST ' macOS (ARM/Intel Universal)' link
								$windowsDownloadLink = $null
								$foundLinks = $downloadDevSection.getElementsByTagName('a') | Where-Object { $_.innerText -match "macOS" }

								if ($foundLinks.Count -gt 0) {
									# Take the first 'macOS (ARM/Intel Universal)' link (most recent)
									$windowsDownloadLink = $foundLinks[0].href
								}

								if (-not $windowsDownloadLink) {
									Write-Error "No download link found for the macOS (ARM/Intel Universal) version in Development versions."
									exit 1
								}

								# Construct the full download URL
								$downloadUrl = $windowsDownloadLink
								#Write-Host "Download URL for macOS (ARM/Intel Universal): $downloadUrl"
								...
								try {
									Start-BitsTransfer -Source $downloadUrl -Destination $destinationFilePath -Priority Foreground
									Write-Host "Dolphin download for macOS (ARM/Intel Universal) completed successfully." -ForegroundColor "Green" -BackgroundColor "Black"
								} catch {
									Write-Error "Failed to download the file using BITS: $_"
									exit 1
								}
